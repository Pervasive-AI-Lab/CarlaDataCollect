...
class CameraSemantic(object):
    def __init__(self,name, parent_actor, position,recorder=None, showWin=False, imgWidth=640, imgHeight=480, fov=110):
        self.NAME = name
        ...
        if position == 'left':
            loc = carla.Location(x=3, y=-1, z=1)
            rot = carla.Rotation(pitch=-15.0, yaw=-35)
        elif position == 'right':
            loc = carla.Location(x=3, y=1, z=1)
            rot = carla.Rotation(pitch=-15.0, yaw=35)
        else:
            loc = carla.Location(x=3, y=0, z=2)
        ...
        # if (self.recorder):
        #     self.recorder.open_input(self.NAME, True)
    ...
    def drawImage(self):
        if self.existsImage:
            cv2.imshow(self.NAME, self.imagetoShow)
            cv2.waitKey(1)

    def getData(self):
        if self.existsImage:
            scaled_image = self.imagetoShow / 255.0
            return scaled_image
        return None
